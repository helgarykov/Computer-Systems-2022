...
int fhistogram(char const *path) {
  ...
  while (fread(&c, sizeof(c), 1, f) == 1) {
    ...
    if ((i % 100000) == 0) {
      ...
      // Potentially more appropriate to lock after merging, idk concurrency is hard
...
void* worker(void *arg) {
  ...
  while (1) {
    char *c;        // pointer to char array
    if (job_queue_pop(jq, (void**)&c) == 0) {
      fhistogram(c);
      free(c);
    } else {
...
int main(int argc, char * const *argv) {
  ...
  while ((p = fts_read(ftsp)) != NULL) {
    ...
    case FTS_D:
      break;
    case FTS_F:
      job_queue_push(&q, (void*)strdup(p->fts_path));
      break;
    default:
      break;
    }
