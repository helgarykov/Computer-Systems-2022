...
/* Structure target: node of tree with lon and lat coordinates */
struct point{
    double lat;
    double lon;
};

/* Function to add a node to the kd-tree */
...
/* Helper-function to calculate the Eucledian distance between two nodes with respect to their lon and lat coordinates */
...
/* Helper-function to return the closest node in the kd-tree to the target point */
struct node_of_tree* closest_point(struct point* target,
                                   struct node_of_tree* candidate_one, 
                                   struct node_of_tree* candidate_two){
    if(candidate_one == NULL) return candidate_two;
    if(candidate_two == NULL) return candidate_one;
...
/* Function to search through the kd-tree for the node that is closest to the target point */
